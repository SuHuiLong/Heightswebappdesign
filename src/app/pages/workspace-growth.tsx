...
import { TrendingUp, Search, Send, Check, History } from 'lucide-react';
...
function getScopePaletteStateForTarget(
  ...
  if (!targetLevel) {
    return { targetLevel: null, step: 'root', ...context };
  }

  if (targetLevel === 'all') {
    return { targetLevel, step: 'root', ...context };
  }
      ...
      return getOrganizationsForRegion(state.region)
        ...
        .map((organization) => ({
          ...
          nextState:
            state.targetLevel === 'organization'
              ? undefined
              : { ...state, step: 'subscriber', organization: organization.id, subscriber: undefined },
        }));
    case 'subscriber':
      return getSubscribersForOrganization(state.organization)
        ...
        .map((subscriber) => ({
          ...
          nextState:
            state.targetLevel === 'subscriber'
              ? undefined
              : { ...state, step: 'device', subscriber: subscriber.id },
        }));
    case 'device':
...
export function GrowthWorkspace() {
  ...
  const scopeLabel = useMemo(
    () => getWorkspaceScopeDisplayLabel('growth', currentScope),
    [currentScope],
  );
  ...
  const getScopeChangeMessage = (scope: ScopeSelection): string => {
    ...
    switch (scope.level) {
      case 'all':
        return 'Now showing growth data for all tenants across all regions.';
      case 'region':
        return `Scope changed to ${getScopeDisplayLabel(scope)}. I can help you identify growth opportunities in this region.`;
      case 'organization':
        return `Now focused on ${getScopeDisplayLabel(scope)}. What opportunities exist for this organization?`;
      case 'subscriber':
        return `Viewing subscriber ${getScopeDisplayLabel(scope)}. I can analyze upsell potential and churn risk for this subscriber.`;
      case 'device':
        return `Viewing gateway device ${getScopeDisplayLabel(scope)}. I can profile usage patterns and recommend upgrades for this gateway.`;
      default:
            ...
            >
              ...
              <div className="flex items-end gap-2">
                ...
                <span className="mb-1 text-sm" style={{ color: 'var(--success)' }}>
                  Opportunities &middot; +18 vs last month
                </span>
              </div>
              <div className="mt-3 grid grid-cols-3 gap-3">
                <div>
                  <div className="text-xs" style={{ color: 'var(--neutral-500)' }}>High Intent</div>
                  <div className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>34</div>
                </div>
                <div>
                  <div className="text-xs" style={{ color: 'var(--neutral-500)' }}>Medium Intent</div>
                  <div className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>58</div>
                </div>
                <div>
                  <div className="text-xs" style={{ color: 'var(--neutral-500)' }}>Low Intent</div>
                  <div className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>35</div>
                </div>
              </div>
              <div className="mt-3 pt-3 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
                <span className="text-xs" style={{ color: 'var(--neutral-500)' }}>Source: Growth Analytics Engine</span>
              </div>
            </div>
          </motion.div>
        );
          ...
          >
            ...
            >
              ...
              <div className="space-y-2">
                {[
                  { severity: 'critical', message: '23 subscribers showing strong churn signals', count: 23 },
                  { severity: 'medium', message: 'Bandwidth saturation detected for 156 users', count: 156 },
                  { severity: 'low', message: '42 new VAS-eligible households identified', count: 42 },
                ].map((alert, i) => (
                  <div
                    key={i}
            ...
            <div className="max-w-3xl mx-auto">
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <div className="space-y-0.5">
                  {WORKSPACES.growth.topQuestions.map((q, i) => (
                  <button
                  key={i}
                  type="button"
                  onMouseDown={(e) => {
